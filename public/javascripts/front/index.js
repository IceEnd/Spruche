(function ($) {
    ...
    var $nav_ul = $('#nav_ul');
    var $mobile_nav_ul = $('#mobile_nav_ul');
    ...
    function init(){
        ...
        switch (window.location.pathname) {
            case '/':
                $nav_ul.attr('data-type', 'home');
                $mobile_nav_ul.attr('data-type', 'home');
                break;
            case '/friendslink':
                $nav_ul.attr('data-type', 'friends');
                $mobile_nav_ul.attr('data-type', 'friends');
                break;
            case '/messageboard':
                $nav_ul.attr('data-type', 'tucao');
                $mobile_nav_ul.attr('data-type', 'tucao');
                break;
            default:
                $nav_ul.attr('data-type', 'home');
                $mobile_nav_ul.attr('data-type', 'home');
                            ...
                            '<article>' +
                              ...
                              '<div class="article-info float-left">'+
                                '<h3 class="article-title-h">' +
                                  '<span class="shine-font-span"></span>' +
                                  '<a href="/article/av'+blogs[b].id+'" data-pjax="true">'+blogs[b].title+'</a>'+
                                '</h3>' +
                                '<div class="article-other-info">' +
                                  '<span><a class="article-author">'+blogs[b].username+'</a></span>' +
                                  '<span><a class="article-comments-num" data-count-type="comments" data-pjax="true">' + blogs[b].commentsView.amount + ' Comments</a></span>' +
                                  '<span><a class="article-view-num">'+blogs[b].view_num+' Views</a></span>' +
                                  '<span><a class="article-date">'+ (new Date(blogs[b].publish_date)).getFullYear()+'-'+((new Date(blogs[b].publish_date)).getMonth()+1)+'-'+(new Date(blogs[b].publish_date)).getDate()+'</a></span>' +
                                '</div>' +
                              '</div>'+
    ...
    $(document).on('click', '.nav_ul_btn', function (){
        $nav_ul.attr('data-type', $(this).attr('data-type'));
        $mobile_nav_ul.attr('data-type', $(this).attr('data-type'));
    });

    $(document).on('click', '.mobile_nav_ul_btn', function (e) {
        stopPropagation(e);
        $nav_ul.attr('data-type', $(this).attr('data-type'));
        $mobile_nav_ul.attr('data-type', $(this).attr('data-type'));
    });
